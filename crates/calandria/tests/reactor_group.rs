//! Static reactor-group routing, termination, and peer-failure tests.

use std::{error::Error, sync::mpsc, time::Duration};

use calandria::{
    Lane, ReactorFailure, ReactorGroup, ReactorGroupMemberExit, ReactorGroupOutcome,
    ReactorGroupSendFailure, ReactorId, ReactorOutcome, ReactorTerminationStatus, RetainedBytes,
    WakeHandle,
};

#[path = "reactor_group/startup.rs"]
mod startup;
#[path = "reactor_group_support/mod.rs"]
mod support;

use support::{
    Command, FirstTurn, group_limits, member, member_with_termination_wake, observations,
    observed_values,
};
